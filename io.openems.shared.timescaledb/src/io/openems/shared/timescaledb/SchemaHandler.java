...
public class SchemaHandler {
	...
	public void applySchema() throws SQLException {
		try (var con = this.dataSource.getConnection();
			...
			// pg_uuidv7 supplies uuid_generate_v7() — time-ordered UUIDs with
			// the same B-tree locality as a sequential BIGINT. Required for
			// sharding-safe IDs on edge / channel without v4's random-insert
			// index bloat.
			st.execute("CREATE EXTENSION IF NOT EXISTS pg_uuidv7");

			// Layer 1 — dimension tables
			// edge.id is a UUID generated by the database.
			st.execute("""
			...
			st.execute("""
					CREATE TABLE IF NOT EXISTS channel_def (
					    id          UUID DEFAULT uuid_generate_v7() PRIMARY KEY,
					    name        VARCHAR NOT NULL UNIQUE,
					    type        VARCHAR NOT NULL CHECK (type IN ('INTEGER','FLOAT','STRING')),
					    unit        VARCHAR,
					    created_at  TIMESTAMPTZ NOT NULL DEFAULT now()
					)""");

			// channel.id is referenced by every row in the data_* hypertables
			// (billions of rows). UUID v7 keeps it sharding-safe; compression's
			// segmentby='channel_id' collapses the 16-byte cost to ~1-2 bytes
			// per row in compressed chunks.
			// ON DELETE CASCADE — deleting a component/channel_def cleans up its
			...
			// Compression — ALTER TABLE is idempotent in TimescaleDB but throws if
			// compression is already enabled, so we swallow the error.
			...
			// === FAST LANE (core = true) ===
			// 1-minute (base, reads raw filtered to core channels)
			...
			// === SLOW LANE (all channels, low + high) ===
			// 15-minute (base, reads raw with no core filter)
			...
			// Asymmetric chunk sizing — aggregate rows are far sparser than raw
			// rows, so larger chunks cost little RAM but slash chunk-catalog
			// overhead and constraint-exclusion planner work.
			// Fast Lane chunk sizing
			...
			// Aggregate refresh policies (start_offset, end_offset, schedule_interval).
			// Refreshes are incremental, touching only the recent tail, so larger
			// chunks don't translate into larger refreshes.
			// Fast Lane refresh policies
			...
			for (var t : new String[] { "integer", "float" }) {
				...
				// Slow Lane retention — None (kept forever; no retention policy)
			}
	...
	private static void createHypertable(java.sql.Statement st, String table, String valueType, String chunkInterval)
		...
		// channel_id is UUID. Compression's segmentby='channel_id' uses
		// dictionary encoding to store each UUID once per chunk, neutralising
		// the 16-byte storage cost.
	...
	private static void createAgg(java.sql.Statement st, String viewName, String bucket,
		...
		// Continuous aggregates live in timescaledb_information.continuous_aggregates,
		// not pg_matviews — skip if already registered there.
	 ...
	 * hypertable. Larger intervals shrink catalog overhead. Idempotent.
	 */
