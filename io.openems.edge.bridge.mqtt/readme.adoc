...
Certificate files may be PEM encoded or PKCS12/JKS stores, detected from the file content. Without a CA certificate file the JVM default CAs are trusted.

* `caCertificateFile` (String): CA certificates for verifying the broker. PEM with one or more certificates, or a JKS/PKCS12 trust store (optional)
* `caCertificatePassword` (String): Password of a PKCS12/JKS trust store, not used for PEM. Without it the store integrity is not checked and password protected PKCS12 entries cannot be read (optional)
* `clientCertificateFile` (String): Client certificate chain for mutual TLS in leaf-first order. PEM, or a PKCS12/JKS key store that also holds the private key (optional)
* `clientKeyFile` (String): PEM private key for the client certificate, PKCS#8 or traditional OpenSSL, plain or encrypted. Not needed if the key is contained in the client certificate file (optional)
* `clientKeyPassword` (String): Password of an encrypted PEM key or of a PKCS12/JKS key store (optional)
