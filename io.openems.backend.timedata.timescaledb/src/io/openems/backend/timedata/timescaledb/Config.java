...
@interface Config {
	...
	@AttributeDefinition(name = "Retention Days", description = "Days to keep raw high-res data before deletion; 0 or less keeps it forever")
	int retentionDays() default 0;

	@AttributeDefinition(name = "Compression Days", description = "Days to wait before compressing raw high-res data")
	...
	@AttributeDefinition(name = "Start date", description = "Reject writes and queries before this date, for example: 2025-12-30; optional", required = false)
	String startDate();

	@AttributeDefinition(name = "End date", description = "Reject writes and queries after this date, for example: 2025-12-31; optional", required = false)
	String endDate();

	@AttributeDefinition(name = "List of blacklisted Channel Addresses", description = "Blacklisted ChannelAddresses which should not be written to the database. e.g. \"kacoCore0/Serialnumber\"")
	String[] blacklistedChannels() default {};

	@AttributeDefinition(name = "List of blacklisted Channel IDs", description = "Blacklisted ChannelIds which should not be written to the database. e.g. \"_PropertyAlias\"")
